function [] = AllYeoParcellate(Subs,GSR,isServer,type,doMSM,varargin)
...
for iSub=1:numel(Subs)
    ...
    for iY=1:numel(YeoSize)
        ...
        if ~isempty(type)
            save(strcat(bDir,num2str(GSR),'/sub',num2str(Subs(iSub)),ParcName,type,NameEnd,'.mat'),'X');
        else
            save(strcat(bDir,num2str(GSR),'/sub',num2str(Subs(iSub)),ParcName,NameEnd,'.mat'),'X');
        end
