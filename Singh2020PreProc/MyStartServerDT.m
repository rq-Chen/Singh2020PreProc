function[]=MyStartServerDT(Subjects,nGSR,copyBase,Atlas)
...
in_dir  = '/data/hcp-zfs/OpenAccess/1200subject/'; % the Directoy where your subject data lives
...
Myc_fcprocess_HCP_dtseries(switches, Subjlist, in_dir, out_dir, tseries)
%disp('running ptSeries')%run parcellated time series
%c_fcprocess_HCP_ptseries(switches, Subjlist, in_dir, out_dir, tseries, Parc)if strcmpi(Atlas(1),'y')

if strcmpi(Atlas(1),'y')
    if strcmpi(Atlas(2:end),'7')
        AllYeoParcellate7net(str2double(Subjlist),nGSR,'y',[],'n');
    else
        AllYeoParcellate(str2double(Subjlist),nGSR,'y',[],'n');
    end
else
