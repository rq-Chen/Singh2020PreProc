...
% All Rights Reserved
function MygetHCPregressorsCompCor_finalRetest(Subject, in_dir, out_dir,tseries)
...
NewDir=fullfile(in_dir,sub,'_3T/RESOURCES/');
StructDir=strcat(NewDir,'Structural_preproc');
    asegname = fullfile(StructDir,'/MNINonLinear/aparc+aseg.nii.gz');
    ...
    rfMRIname = fullfile(NewDir, strcat(tseries{1},'_PostFix'), '/MNINonLinear/Results', tseries{1}, [tseries{1} '_hp2000_clean.nii.gz']);
    ...
    for t=1:length(tseries)
        ...
        if ~exist(outputname,'file')
            ...
            for m=1:2
                ...
                %% PCA
                [coeff, score, latent] = princomp(R_TC,'econ');
                regs = [regs coeff(:,1:5)];
            end
