function[]=MyStartServerDTretest(Subjects,nGSR,copyBase)
...
out_base=strcat(NewBase,'HCP/Out/');
out_dir=strcat(NewBase,'HCP/GSR',num2str(nGSR),'/Out');
    ...
    for ii=1:numel(Subjlist)
        for jj=1:numel(RrName)  %% Scan Sessions
            ...
            for kk=1:numel(VarNames)
                ...
                copyfile(strcat(out_base,tExtend,cc),strcat(out_dir,tExtend,cc));
            end
        end
    end
end
...
in_dir='/data/hcpdb/archive/HCP_Retest/arc001/';
...
addpath(strcat(NewBase,'restingState/FSLNets'))
addpath(strcat(NewBase,'restingState/fieldtrip'))
addpath(strcat(NewBase,'restingState/fieldtrip/utilities'))
addpath(strcat(NewBase,'restingState/fieldtrip/external/freesurfer'))
addpath(strcat(NewBase,'restingState/fieldtrip/filio'))
addpath(strcat(NewBase,'restingState/fieldtrip/fileio/freesurfer'))
...
%addpath('/scratch1/MitchJeffers/restingState/fieldtrip/fileio/special')
...
tseries=cell(1,numel(RrName));
for j=1:numel(RrName)
    tseries{j}=strcat(BbName,RrName{j});
end
...
for i = 1:length(Subjlist)
    ...
    find_FDretest(in_dir, out_dir, Subjlist{i},tseries);
    find_DVARretest(in_dir, out_dir, Subjlist{i}, tseries);
    ...
    MygetHCPregressorsCompCor_finalRetest(Subjlist{i}, in_dir, out_dir,tseries)
end
...
%Parc.key = '/scratch1/MitchJeffers/restingState/Gordon333SRI_ParcelsKey.csv'                                  
...
%Parc.ordered = 1
 ...
 Myc_fcprocess_HCP_dtseriesRetest(switches, Subjlist, in_dir, out_dir, tseries)
%disp('running ptSeries')%run parcellated time series
%c_fcprocess_HCP_ptseries(switches, Subjlist, in_dir, out_dir, tseries, Parc)
d112817ParcellateServerRetest(str2double(Subjlist),nGSR,[],Task);
end
