function[]=MyStartServerDT_Retest(Subjects,nGSR,copyBase,Atlas)
...
out_base=strcat('/scratch2/Singh/HCP_Retest/Out/');
out_dir=strcat('/scratch2/Singh/HCP_Retest/GSR',num2str(nGSR),'/Out');
...
for i = 1:length(Subjlist)
    BuildFileStructure(out_dir, Subjlist{i}, tseries)
        ...
        for jj=1:numel(RrName)  %% Scan Sessions
            tExtend=strcat(out_dir,'/',Subjlist{i},'/Results/',BbName,RrName{jj},'/');
    ...
    getHCPregressorsCompCor_final(Subjlist{i}, in_dir, out_dir,tseries)
end
...
%Parc.ordered = 1
 ...
 Myc_fcprocess_HCP_dtseries_Retest(switches, Subjlist, in_dir, out_dir, tseries)
...
AllYeoParcellate7net_Retest(str2double(Subjlist),nGSR,'y',[],'n');
    else        
AllYeoParcellate_Retest(str2double(Subjlist),nGSR,'y',[],'n');
    end
else
d112817ParcellateServer_Retest(str2double(Subjlist),nGSR,[],Atlas);
end
end
