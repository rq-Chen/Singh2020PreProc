function[]=MyStartServerDTSuperClean(Subjects,nGSR,copyBase,Atlas)
...
out_base=strcat('/scratch2/Singh/HCP/SuperClean/Out/');
out_dir=strcat('/scratch2/Singh/HCP/SuperClean/GSR',num2str(nGSR),'/Out');
...
for i = 1:length(Subjlist)
    ...
    getHCPregressorsCompCor_finalSuperClean(Subjlist{i}, in_dir, out_dir,tseries)
end
...
%Parc.ordered = 1
 ...
 Myc_fcprocess_HCP_dtseriesSuperClean(switches, Subjlist, in_dir, out_dir, tseries)
%disp('running ptSeries')%run parcellated time series
%c_fcprocess_HCP_ptseries(switches, Subjlist, in_dir, out_dir, tseries, Parc)
d112817ParcellateServerSuperClean(str2double(Subjlist),nGSR,[],Atlas);
end
