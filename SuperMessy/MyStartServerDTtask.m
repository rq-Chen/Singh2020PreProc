function[]=MyStartServerDTtask(Subjects,nGSR,copyBase,Task)
...
RrName={'LR','RL'};
BbName=strcat('tfMRI_',upper(Task));
...
out_dir=strcat('/scratch2/Singh/HCP/GSR',num2str(nGSR),'/Out');

if strcmpi(copyBase(1),'y')
    for ii=1:numel(Subjlist)
        for jj=1:numel(RrName)  %% Scan Sessions
            tExtend=strcat('/',Subjlist{ii},'/Results/',BbName,'_',RrName{jj},'/');
            ...
            for kk=1:numel(VarNames)
                if kk<5
                    cc=strcat('/',Subjlist{ii},'_',BbName,'_',RrName{jj},'_',VarNames{kk});
                else
                    cc=VarNames{kk};
                end
...
for j=1:numel(RrName)
    tseries{j}=strcat(BbName,'_',RrName{j});
end

%tseries = RestNameSetup(Data.Type);%sets up the tseries name used in either DMCC or HCP
switches = MySwitchSetupTask(Data.Type);%sets up the parameters CHECK BEFORE RUNNING
switches.GSR=nGSR;
...
for i = 1:length(Subjlist)
    ...
    MygetHCPregressorsCompCor_finalTask(Subjlist{i}, in_dir, out_dir,tseries)
end
...
d112817ParcellateServer(str2double(Subjlist),nGSR,[],Task);
end
