function switches = MySwitchSetupTask(DataType)
    if strcmp( DataType, 'HCP' )%if it's HCP Data use these paramerters 
        ...
        switches.FDcut = 0.3; % threshold from petersen values to cut (recommend 0.04) CAROLINA: check this value!!
        switches.DVARcut = 1.1; % percent above median to cutoff. zero means no cutoff (recommend 1.05)
        switches.permutemask = 0; % randomly scrambles the tmask
        switches.figs = 0; % generate figs to QC processing
        switches.minframes = 150; % (recommend 400, unless you are afraid to throw away 1/4 subjects)
