...
% All Rights Reserved
function getHCPregressorsCompCor_final_NoFIX(Subject, in_dir, out_dir,tseries)
...
%load(fullfile(out_dir, 'Subjlist.mat')) % cell array of subject IDs (strings)
    ...
    rfMRIname = fullfile(in_dir, sub, '/MNINonLinear/Results', tseries{1}, [tseries{1} '.nii.gz']);
 ...
 %      subplot(2,2,4);imagesc(CSF2(:,:,48));title('eroded image');
    ...
    for t=1:length(tseries)
        ...
        outputname = fullfile(out_dir, sub,'Results', tseries{t}, '/WM_CSF_CompCor_regs.txt');
        
        if ~exist(outputname,'file')
            
            rfMRIname = fullfile(in_dir, sub, '/MNINonLinear/Results', tseries{t}, [tseries{t} '.nii.gz']);
            ...
            if savemeanregs
                fid = fopen(fullfile(out_dir, sub, 'Results', tseries{t}, 'WM_CSF_GM_regs.txt'),'w');
